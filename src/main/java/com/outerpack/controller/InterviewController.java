...
public class InterviewController {
    ...
    public Result getInterviewById(@ApiParam(value = "面试记录ID",example = "1")@PathVariable("ID")int ID){
    ...
    @GetMapping("/getInterviewByHRID/{ID}")
    public Result getInterviewByHRID(@ApiParam(value = "HR的ID",example = "1")@PathVariable("ID") int ID,@ApiParam(value = "页码",example = "2")@RequestParam(defaultValue = "1")Integer pageNum){
    ...
    public Result getInterviewByCandidateId(@ApiParam(value = "面试者ID",example = "1")@PathVariable("ID")int ID,@ApiParam(value = "页码",example = "2")@RequestParam(defaultValue = "1")Integer pageNum){

        PageResult<Interview> list = interviewService.getInterviewByCandidateId(ID, pageNum);
        if(list!=null)return Result.success("当前页"+ID+"号的面试者的面试记录",list);
        else return Result.error("查询失败");
    }

    @ApiOperation("获取所有面试记录")
    @GetMapping("/getAllInterview")
    public Result getAllInterview(@ApiParam(value = "页码",example = "1")@RequestParam(defaultValue = "1")Integer pageNum){
        PageResult<Interview> allInterview = interviewService.getAllInterview(pageNum);
        if(allInterview==null)return Result.error("无面试记录");
        else return Result.success("当前页面试记录列表",allInterview);
    }
